...
class VSP(object):
    def __init__(self, filename):
        ...
        if signature != VSP_SIGNATURE:
            raise FormatException("The VSP file '" + filename + "' has a bad signature " + signature)
        if version != VSP_VERSION:
            raise FormatException("The VSP file '" + filename + "' has a bad version "  + str(version))
    ...
    def dumpTiles(self):
        ...
        tileImage = Image.new("RGBA", (20 * 16, (self.tileCount // 20 + 1) * 16))
    ...
    def dumpObs(self):
        ...
        obsImage = Image.new("RGBA", (20 * 16, (self.obsCount // 20 + 1) * 16))
...
class Entity(object):
    def __init__(self, f):
        ...
        self.y = f.readShort()
        faceID = f.readByte()
        if faceID < 0 or faceID > len(ENTITY_DIR):
            faceID = 0
        self.direction = ENTITY_DIR[faceID]
...
class Map(object):
    def __init__(self, filename):
        ...
        if signature != MAP_SIGNATURE:
            raise FormatException("The MAP file '" + filename + "' has a bad signature " + signature)
        # Verify the map is the right version
        if version != MAP_VERSION:
            raise FormatException("The MAP file '" + filename + "' has a bad version " + str(version))
    ...
    def dumpZoneDummyImage(self):
        ...
        image = Image.new("RGBA", (20 * 16, (zoneCount // 20 + 1) * 16))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2: 
        print('v3tiled3: no input files')
        print('Usage: v3tiled MAPFILE [MAPFILE ...]')
        print('')
        print('Convert maped3 formats into tmx + png files.')
        print('')
        print('MAPFILE: a map to convert (v3tiled will also require the VSP the map uses)')
    else:
        print('v3tiled3')
        print('')
